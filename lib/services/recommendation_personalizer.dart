  ...
  String _buildText(
    ...
    switch (id) {
      case 'sleep':
        ...
        return switch (v % 4) {
          0 =>
            'За последние 14 дней ${_daysRu(n)} со сном ниже $t/10 — попробуйте $step $when.',
          1 =>
            'По вашим данным за две недели: ${_daysRu(n)} со сном слабее $t/10. Мягкий шаг — $step.',
          2 =>
            'Сон по отметкам: ${_daysRu(n)} ниже $t/10 за 14 дней. Можно начать с: $step ($when).',
          _ =>
            'За 14 дней видно ${_daysRu(n)} со сном ниже $t/10. Попробуйте $step.',
        };
      case 'energy':
      ...
      case 'mood_notes':
        ...
        return switch (v % 3) {
          0 =>
            'За 14 дней ${_daysRu(nm)} с настроением ниже ${stats.moodThresholdDisplay}/10; заметок за период — $nn. Короткая запись 3–5 строк в день часто помогает увидеть паттерн.',
          1 =>
      ...
      case 'mood':
        ...
        return switch (v % 3) {
          0 =>
            'За 14 дней ${_daysRu(n)} с настроением ниже ${stats.moodThresholdDisplay}/10.$extra',
          1 =>
          ...
          _ =>
            'Настроение по дням: ${_daysRu(n)} ниже ${stats.moodThresholdDisplay}/10 за 14 дней. Мягкий фокус — сон, движение и вода.$extra',
        };
      case 'meds':
        return switch (v % 3) {
          0 => 'По календарю есть приём препаратов — отмечайте слоты, чтобы видеть связь с самочувствием.',
      ...
      case 'doctor':
        return switch (v % 3) {
          0 => 'В календаре есть визит — хороший повод коротко описать симптомы и сон за последние дни.',
          1 => 'Перед визитом к врачу можно выгрузить из заметок 3–5 фактов: сон, энергия, настроение — как вы их чувствуете.',
          _ => 'Запланированный визит — удобная точка, чтобы согласовать режим и переносимость.',
        };
      case 'notes_regular':
        final td = stats.trackedDays.clamp(0, 14);
        return switch (v % 3) {
          0 =>
            'За 14 дней с записями — ${_daysRu(td)}; регулярные короткие отметки делают картину стабильнее.',
          1 =>
      ...
      case 'default':
        return switch (v % 2) {
          0 => 'Продолжайте короткие отметки — по мере накопления дней подсказки станут точнее.',
          _ => 'Чем ровнее неделя записей, тем меньше «шума» в советах.',
        };
      default:
        return fallback;
    }
